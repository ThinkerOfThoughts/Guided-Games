		...
		axios.get(base_url + 'SearchDestinyPlayer/' + type + '/' + account, options)
			...
			axios.get(base_url + type + '/Account/' + ID + '/Character/0/Stats/?modes=None', options)
				...
				*/
				...
				function getRole(){
					var state = document.getElementById("role");
					if(state.value === "mentor"){
						user_role = "Mentor";
					}
					else if(state.value === "mentee")
					{
						user_role = "Mentee";
					}
					console.log("USER ROLE FROM FUNC:", user_role);	
				}
				console.log("Role outside function:", user_role);
